...
pub async fn init_db(client: &Client) -> Result<()> {
    ...
    client.batch_execute(schema_v1).await.context("Failed to execute initial schema")?;
    
    let schema_v2 = include_str!("../migrations/002_api_keys.sql");
    client.batch_execute(schema_v2).await.context("Failed to execute api_keys schema")?;
    
    let schema_v3 = include_str!("../migrations/003_domains_and_history.sql");
    client.batch_execute(schema_v3).await.context("Failed to execute domains schema")?;

    let schema_v4 = include_str!("../migrations/004_rollback_support.sql");
    client.batch_execute(schema_v4).await.context("Failed to execute rollback support schema")?;

    println!("Database initialized (Migration v4 successful).");
    Ok(())
}

pub async fn import_sources(client: &Client, config: &SourcesConfig, force: bool) -> Result<(Vec<String>, Vec<String>)> {
    let domain = config.domain.as_deref().unwrap_or(DEFAULT_DOMAIN);
    
    // Ensure domain exists
    client.execute("INSERT INTO domains (name) VALUES ($1) ON CONFLICT DO NOTHING", &[&domain]).await?;
    ...
    for source in &config.sources {
        ...
        println!("Synced Source: {} (ID: {}) [Domain: {}]", source.name, source_id, domain);
        ...
        for table in &source.tables {
            ...
            let table_rows = client.query(
                "INSERT INTO tables (source_id, name, schema_name, partition_column)
                 ...
                 RETURNING id",
                &[&source_id, &table.name, &table.schema, &table.partition_column],
            ).await.context("Failed to upsert table")?;
            ...
            if !active_column_names.is_empty() {
                ...
                ).await.context("Failed to prune columns")?;
                if deleted > 0 {
                    println!("  Pruned {} obsolete columns from table {}.{}", deleted, table.schema, table.name);
                }
            }
        }
        ...
        if !active_table_ids.is_empty() {
            ...
            ).await.context("Failed to prune tables")?;
            if deleted > 0 {
                println!("  Pruned {} obsolete tables from source {}", deleted, source.name);
            }
        }
    }
    ...
    if !active_source_ids.is_empty() {
        ...
        let deleted = client.execute(
            ...
            &[&domain, &active_source_ids],
        ).await.context("Failed to prune sources")?;
        if deleted > 0 {
            println!("Pruned {} obsolete sources from domain {}.", deleted, domain);
        }
    } else {
        if config.sources.is_empty() {
            ...
            let to_delete = client.query("SELECT name FROM sources WHERE domain_name = $1", &[&domain]).await?;
            for row in to_delete {
                deleted_sources.push(row.get(0));
            }

            let deleted = client.execute("DELETE FROM sources WHERE domain_name = $1", &[&domain]).await.context("Failed to prune all sources in domain")?;
            if deleted > 0 {
                 println!("Pruned all {} sources from domain {} (config was empty).", deleted, domain);
            }
        }
    }

    println!("Synchronization for domain '{}' completed successfully.", domain);
...
pub async fn get_domain_version(client: &Client, domain: &str) -> Result<i32> {
    ...
    match row {
        ...
        None => {
            ...
            client.execute("INSERT INTO domains (name, version) VALUES ($1, 1) ON CONFLICT DO NOTHING", &[&domain]).await?;
            Ok(1)
        }
    }
}

pub async fn increment_domain_version(client: &Client, domain: &str, expected_version: i32) -> Result<i32> {
    let rows = client.execute(
        "UPDATE domains SET version = version + 1 WHERE name = $1 AND version = $2",
        &[&domain, &expected_version],
    ).await.context("Failed to increment domain version")?;

    if rows == 0 {
        return Err(anyhow!("Optimistic locking failure: Domain '{}' version has changed (expected v{})", domain, expected_version));
    }
...
pub async fn get_history_config(client: &Client, domain: &str, version: i32) -> Result<String> {
    ...
    match row {
        Some(r) => Ok(r.get(0)),
        None => Err(anyhow!("Version {} not found for domain '{}'", version, domain)),
    }
}

pub async fn get_all_sources(client: &Client, domain: Option<&str>) -> Result<models::SourcesConfig> {
    let domain = domain.unwrap_or(DEFAULT_DOMAIN);
    let rows = client
        .query("SELECT id, name, type, url FROM sources WHERE domain_name = $1", &[&domain])
        .await
    ...
    Ok(models::SourcesConfig { domain: Some(domain.to_string()), sources })
}
