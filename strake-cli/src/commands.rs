...
pub async fn init(template: Option<String>) -> Result<()> {
    ...
    println!("Initialized sources.yaml with {:?} template", template.unwrap_or_else(|| "default".to_string()));
    Ok(())
}
...
pub async fn validate(file_path: &str, offline: bool) -> Result<()> {
    println!("{} {} {}", "[Config:".dimmed(), file_path.yellow(), "] Validating...".bold().cyan());
    let config = parse_yaml(file_path)?;
    
    println!("Structure is valid.");
    
    if offline {
        println!("{}", "Skipping semantic validation (offline mode).".dimmed());
        return Ok(());
    }
    ...
    for source in config.sources {
        ...
        match validate_source(&source).await {
            Ok(_) => pb.finish_with_message(format!("{} source '{}': OK", "✔".green(), source.name)),
            Err(e) => {
...
async fn validate_file_source(url: &str) -> Result<()> {
    ...
    let file = tokio::fs::File::open(path).await.context(format!("Failed to open file: {}", path_str))?;
    let mut reader = BufReader::new(file);
    let mut buffer = String::new();
    
    // Read up to 3 lines to verify readability and header presence
    for _ in 0..3 {
        let bytes = reader.read_line(&mut buffer).await.context("Failed to read line from file")?;
        if bytes == 0 {
            break; // EOF
        }
    }
...
async fn validate_postgres_source(source: &models::SourceConfig, url: &str) -> Result<()> {
    ...
    for table in &source.tables {
        ...
        if !table_exists {
            return Err(anyhow!("Table '{}.{}' not found in upstream", table.schema, table.name));
        }
        ...
        for col in &table.columns {
            ...
            if !col_exists {
                 return Err(anyhow!("Column '{}' not found in table '{}.{}'", col.name, table.schema, table.name));
            }
        }
    }
...
async fn validate_contracts(sources_yaml: &str, contracts_path: &str) -> Result<()> {
    ...
    let response = client.post(format!("{}/validate", api_url))
    ...
    let result: strake_core::models::ValidationResponse = response.json().await
...
pub async fn apply(client: &Client, file_path: &str, force: bool, dry_run: bool, expected_version: Option<i32>) -> Result<()> {
    println!("{} {} {}", "[Config:".dimmed(), file_path.yellow(), "] Applying configuration...".bold().cyan());
    ...
    let new_version = db::increment_domain_version(client, domain, current_version_to_update).await
        .context("Failed to increment domain version. Another user may have modified the domain.")?;
    ...
    db::log_apply_event(client, domain, new_version, &user_id, &added, &deleted, &config_hash, &raw_yaml).await?;

    println!("{} configuration applied successfully to domain '{}' (New version: {}).", "✔".green(), domain.bold(), format!("v{}", new_version).yellow());
    Ok(())
}

pub async fn rollback(client: &Client, domain: &str, to_version: i32) -> Result<()> {
    println!("{} Rolling back domain '{}' to version {}...", "⟲".yellow(), domain.bold(), format!("v{}", to_version).yellow());
    ...
    db::log_apply_event(client, domain, new_version, &user_id, &added, &deleted, &config_hash, &config_yaml).await?;

    println!("{} Domain '{}' rolled back successfully to version {} (New version: {}).", "✔".green(), domain.bold(), format!("v{}", to_version).yellow(), format!("v{}", new_version).yellow());
    Ok(())
}
...
async fn diff_internal(client: &Client, file_path: &str) -> Result<()> {
    ...
    println!("{} {} {} domain '{}'...", "[Config:".dimmed(), file_path.yellow(), "] Computing difference for".bold().cyan(), domain.unwrap_or("default").bold());

    let mut changes = 0;

    // Compare sources
    for local_source in &local_config.sources {
        match db_config.sources.iter().find(|s| s.name == local_source.name) {
            None => {
    ...
    for db_source in &db_config.sources {
        if !local_config.sources.iter().any(|s| s.name == db_source.name) {
    ...
    if changes == 0 {
        println!("{}", "No changes detected (schema matches metadata store).".dimmed());
    } else {
...
fn diff_sources(local: &models::SourceConfig, db: &models::SourceConfig) -> usize {
    ...
    if local.source_type != db.source_type {
        println!("  ~ MODIFY source {} type: {} -> {}", local.name, db.source_type, local.source_type);
        changes += 1;
    }
    
    if local.url != db.url {
        println!("  ~ MODIFY source {} url: {:?} -> {:?}", local.name, db.url, local.url);
        changes += 1;
    }

    // Tables
    for local_table in &local.tables {
        match db.tables.iter().find(|t| t.name == local_table.name && t.schema == local_table.schema) {
            None => {
                println!("  {} ADD table: {}.{}", "+".green(), local_table.schema.dimmed(), local_table.name.bold());
                changes += 1;
            }
            Some(db_table) => {
    ...
    for db_table in &db.tables {
        if !local.tables.iter().any(|t| t.name == db_table.name && t.schema == db_table.schema) {
            println!("  {} DELETE table: {}.{}", "-".red(), db_table.schema.dimmed(), db_table.name.bold());
            changes += 1;
        }
    }
...
fn diff_tables(local: &models::TableConfig, db: &models::TableConfig) -> usize {
    ...
    if local.partition_column != db.partition_column {
        println!("    ~ MODIFY table {}.{} partition_column: {:?} -> {:?}", local.schema, local.name, db.partition_column, local.partition_column);
        changes += 1;
    }
        ...
        match db.columns.iter().find(|c| c.name == local_col.name) {
            ...
            Some(db_col) => {
                if local_col.data_type != db_col.data_type {
                    println!("    ~ MODIFY column {} type: {} -> {}", local_col.name, db_col.data_type, local_col.data_type);
                    changes += 1;
                }
...
pub fn parse_yaml(path: &str) -> Result<models::SourcesConfig> {
    let raw_content = fs::read_to_string(path).context(format!("Failed to read config file: {}", path))?;
...
pub async fn search(source: &str, file_path: &str, domain: Option<&str>) -> Result<()> {
    ...
    println!("{} {} {} source '{}' [Domain: {}]...", "[Config:".dimmed(), file_path.yellow(), "] Searching for tables in".bold().cyan(), source.bold(), domain.bold());
    ...
    println!("\nUse 'strake-cli add {} <schema>.<name>' to import a specific table.", source);
    Ok(())
}

pub async fn add(source: &str, table_full_name: &str, domain: Option<&str>, output_path: &str) -> Result<()> {
    let domain = domain.unwrap_or("default");
    println!("{} {} {} table '{}' from source '{}' into domain '{}'...", "[Config:".dimmed(), output_path.yellow(), "] Importing".bold().cyan(), table_full_name.bold(), source.bold(), domain.bold());
    ...
    let mut current_config = if std::path::Path::new(output_path).exists() {
        ...
        strake_core::models::SourcesConfig { domain: Some(domain.into()), sources: vec![] }
    };
    ...
    if let Some(imported_source) = imported_config.sources.first() {
        if let Some(target_source) = current_config.sources.iter_mut().find(|s| s.name == source) {
            ...
            for t in &imported_source.tables {
                if let Some(existing_table) = target_source.tables.iter_mut().find(|existing| existing.name == t.name && existing.schema == t.schema) {
    ...
    println!("{} Successfully added '{}' to {}.", "✔".green(), table_full_name.bold(), output_path.yellow());
    Ok(())
}

pub async fn introspect(source: &str, file_path: &str, _registered: bool, _client: Option<&Client>) -> Result<()> {
    search(source, file_path, None).await
}

pub async fn test_connection(file_path: &str) -> Result<()> {
    println!("{} {} {}", "[Config:".dimmed(), file_path.yellow(), "] Testing connections...".bold().cyan());
    let content = fs::read_to_string(file_path).context("Failed to read config file")?;
    let config: models::SourcesConfig = serde_yaml::from_str(&content).context("Failed to parse YAML structure")?;
    ...
    for source in config.sources {
        ...
        match validate_source(&source).await {
            Ok(_) => pb.finish_with_message(format!("{} source '{}': OK", "✔".green(), source.name)),
            Err(e) => pb.finish_with_message(format!("{} source '{}': FAILED - {}", "✘".red(), source.name, e)),
        }
    }
    Ok(())
}
...
pub async fn describe(client: &Client, file_path: &str, domain: Option<&str>) -> Result<()> {
    println!("{} {} {} domain '{}':", "[Config:".dimmed(), file_path.yellow(), "] Current Configuration in Metadata Store for".bold().cyan(), domain.unwrap_or("default").bold());
    ...
    for source in config.sources {
        println!("{} {} (Type: {})", "Source:".bold().blue(), source.name.bold(), source.source_type.dimmed());
        if let Some(url) = &source.url {
            println!("  {} {}", "URL:".dimmed(), url);
        }
        for table in source.tables {
            println!("  {} {}.{}", "Table:".bold().cyan(), table.schema.dimmed(), table.name.bold());
            ...
            for col in table.columns {
                ...
                if col.primary_key { attribs.push("PK".red().to_string()); }
                if col.unique { attribs.push("UNIQUE".yellow().to_string()); }
                if col.is_not_null { attribs.push("NOT NULL".dimmed().to_string()); }
                
                let attrib_str = if attribs.is_empty() { String::new() } else { format!(" [{}]", attribs.join(", ")) };
                
                println!("      {} {}: {}{}", "•".cyan(), col.name.bold(), col.data_type, attrib_str);
            }
        }
        println!();
    }
...
pub async fn list_domains(client: &Client) -> Result<()> {
    let rows = client.query("SELECT name, version, created_at FROM domains ORDER BY name", &[]).await?;
    println!("{:<20} {:<10} {:<20}", "DOMAIN".bold(), "VERSION".bold(), "CREATED AT".bold());
    println!("{}", "-".repeat(50).dimmed());
    for row in rows {
        let name: String = row.get(0);
        let version: i32 = row.get(1);
        let created_at: chrono::DateTime<chrono::Utc> = row.get(2);
        println!("{:<20} v{:<9} {}", name.bold().cyan(), version.yellow(), created_at.format("%Y-%m-%d %H:%M:%S").dimmed());
    }
    Ok(())
}
...
pub async fn show_domain_history(client: &Client, domain: String) -> Result<()> {
    ...
    println!("{} '{}':", "Apply History for Domain".bold().cyan(), domain.bold());
    println!("{:<10} {:<15} {:<20} {:<20}", "VERSION".bold(), "USER".bold(), "TIMESTAMP".bold(), "CHANGES".bold());
