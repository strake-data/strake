...
impl StrakeQueryExecutor for RemoteBackend {
    ...
    async fn trace(&mut self, query: &str) -> anyhow::Result<String> {
        ...
        let pretty_explain = arrow::util::pretty::pretty_format_batches(&explain_batches)?.to_string();
        Ok(pretty_explain)
    }
    ...
    async fn describe(&mut self, table_name: Option<String>) -> anyhow::Result<String> {
        ...
        if include_schema {
             ...
             for batch in &batches {
                 ...
                 let schema_col = batch.column(schema_col_idx).as_any().downcast_ref::<arrow::array::BinaryArray>().ok_or(anyhow::anyhow!("table_schema not binary"))?;
                     ...
                     if schema_col.is_valid(i) {
                         ...
                         let schema = if let Ok(msg) = arrow::ipc::root_as_message(buffer.as_slice()) {
                             if let Some(s) = msg.header_as_schema() {
                                 ...
                                 arrow::ipc::root_as_schema(buffer.as_slice()).map_err(|e| anyhow::anyhow!("Invalid IPC schema (message header mismatch): {}", e))?
                             }
                         } else {
                             // Not a message, try as raw schema
                             arrow::ipc::root_as_schema(buffer.as_slice()).map_err(|e| anyhow::anyhow!("Invalid IPC schema (raw): {}", e))?
                         };
                         ...
                         let rb = RecordBatch::try_new(output_schema.clone(), vec![Arc::new(name_array), Arc::new(type_array)])?;
