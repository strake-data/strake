...
use datafusion::sql::unparser::dialect::{
    Dialect as UnparserDialect, 
    PostgreSqlDialect, MySqlDialect, SqliteDialect, DefaultDialect
};
...
mod tests {
    ...
    async fn test_plan() -> LogicalPlan {
        ...
        let _ = ctx.register_table("users", std::sync::Arc::new(datafusion::datasource::empty::EmptyTable::new(std::sync::Arc::new(schema))));
        
        ctx.table("users").await.unwrap()
            .filter(col("id").gt(lit(10))).unwrap()
            .select(vec![col("name")]).unwrap()
