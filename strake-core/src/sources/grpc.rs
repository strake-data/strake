...
use crate::config::{SourceConfig, TableConfig, RetrySettings};
...
impl SourceProvider for GrpcSourceProvider {
    ...
    async fn register(
        ...
        let catalog = context.catalog(catalog_name).ok_or(anyhow::anyhow!("Catalog not found"))?;
         
        if catalog.schema("public").is_none() {
             catalog.register_schema("public", Arc::new(MemorySchemaProvider::new()))?;
        }
        
        let provider = Arc::new(GrpcTableProvider::new(
             grpc_config,
             schema,
        ));
...
impl TableProvider for GrpcTableProvider {
    ...
    fn schema(&self) -> arrow::datatypes::SchemaRef { self.schema.clone() }
...
impl GrpcExec {
    fn new(
        ...
        Self { config, schema: projected_schema, projection, limit, cache }
    }
}
...
impl DisplayAs for GrpcExec {
    fn fmt_as(&self, _t: datafusion::physical_plan::DisplayFormatType, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "GrpcExec: url={}/{}", self.config.url, self.config.method)
    }
}

impl ExecutionPlan for GrpcExec {
    fn name(&self) -> &str { "GrpcExec" }
    fn as_any(&self) -> &dyn std::any::Any { self }
    fn schema(&self) -> arrow::datatypes::SchemaRef { self.schema.clone() }
    fn properties(&self) -> &PlanProperties { &self.cache }
    fn children(&self) -> Vec<&Arc<dyn ExecutionPlan>> { vec![] }
    fn with_new_children(self: Arc<Self>, _: Vec<Arc<dyn ExecutionPlan>>) -> datafusion::error::Result<Arc<dyn ExecutionPlan>> {
        Ok(self)
    }
    
    fn execute(&self, _partition: usize, _context: Arc<datafusion::execution::TaskContext>) -> datafusion::error::Result<SendableRecordBatchStream> {
        ...
        let stream = futures::stream::once(async move {
            ...
            let descriptor_path = config.descriptor_set.ok_or_else(|| 
                datafusion::error::DataFusionError::Execution("No descriptor_set provided. Reflection not yet supported.".to_string())
            )?;
            
            let bytes = std::fs::read(&descriptor_path)
                .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to read descriptor file: {}", e)))?;
            
            let pool = DescriptorPool::decode(bytes.as_slice())
                .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to decode descriptor pool: {}", e)))?;
            ...
            let service = pool.get_service_by_name(service_name).ok_or_else(||
                 datafusion::error::DataFusionError::Execution(format!("Service '{}' not found in descriptor", service_name))
            )?;
            
            let method = service.methods().find(|m| m.name() == method_name).ok_or_else(||
                 datafusion::error::DataFusionError::Execution(format!("Method '{}' not found in service '{}'", method_name, service_name))
            )?;
            ...
            if let Some(json_body) = &config.request_body {
                let json_val: serde_json::Value = serde_json::from_str(json_body)
                    .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Invalid JSON request body: {}", e)))?;
                ...
                request_msg = input_desc.clone().deserialize(deserializer)
                    .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to map JSON to Protobuf message: {}", e)))?;
            }
            ...
            let endpoint = tonic::transport::Endpoint::from_shared(url)
                 .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Invalid URL: {}", e)))?;
                 
            let channel = endpoint.connect().await
                 .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to connect: {}", e)))?;
                 
            let mut client = tonic::client::Grpc::new(channel);
            
            let codec = DynamicCodec::new(input_desc, output_desc);
            let path_str = format!("/{}/{}", service_name, method_name);
            let path = path_str.parse::<tonic::transport::Uri>()
                .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Invalid path URI: {}", e)))?
                .into_parts()
                .path_and_query
                .ok_or_else(|| datafusion::error::DataFusionError::Execution("Missing path in URI".to_string()))?;

            // Assuming Unary for now
            let request = tonic::Request::new(request_msg);
            
            let response = client.unary(request, path, codec).await
                .map_err(|e| datafusion::error::DataFusionError::Execution(format!("gRPC call failed: {}", e)))?;
            ...
            let json_val = serde_json::to_value(&response_msg)
                 .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to serialize response to JSON: {}", e)))?;
            ...
            let final_schema = if schema.fields().is_empty() {
                ...
                let inferred = arrow::json::reader::infer_json_schema_from_iterator(values)
                     .map_err(|e| datafusion::error::DataFusionError::Execution(format!("Failed to infer schema: {}", e)))?;
                Arc::new(inferred)
            } else {
                schema.clone()
            };
...
impl DynamicCodec {
    fn new(request_desc: MessageDescriptor, response_desc: MessageDescriptor) -> Self {
        Self { request_desc, response_desc }
    }
}
...
impl Codec for DynamicCodec {
    ...
    fn encoder(&mut self) -> Self::Encoder {
        DynamicEncoder { desc: self.request_desc.clone() }
    }
...
impl Encoder for DynamicEncoder {
    ...
    fn encode(&mut self, item: Self::Item, buf: &mut EncodeBuf<'_>) -> Result<(), Self::Error> {
        use prost::Message;
        item.encode(buf).map_err(|e| Status::internal(format!("Failed to encode: {}", e)))
    }
}
...
impl Decoder for DynamicDecoder {
    ...
    fn decode(&mut self, buf: &mut DecodeBuf<'_>) -> Result<Option<Self::Item>, Self::Error> {
        ...
        msg.merge(buf).map_err(|e| Status::internal(format!("Failed to decode: {}", e)))?;
        Ok(Some(msg))
    }
}
...
fn create_schema_from_config(config: &GrpcSourceConfig) -> Result<arrow::datatypes::SchemaRef> {
    if let Some(cols) = &config.columns {
        let fields: Result<Vec<arrow::datatypes::Field>, _> = cols.iter().map(|c| {
...
mod tests {
    ...
    fn test_config_parsing_full() {
        ...
        assert_eq!(config.descriptor_set, Some("/path/to/descriptor.bin".to_string()));
    ...
    fn test_schema_creation() {
        let config = GrpcSourceConfig {
            ...
            descriptor_set: None,
            columns: Some(vec![
                ColumnConfig { name: "col1".to_string(), data_type: "Utf8".to_string(), nullable: true },
                ColumnConfig { name: "col2".to_string(), data_type: "Int64".to_string(), nullable: false },
            ]),
            tables: None,
        };
    ...
    fn test_invalid_type() {
         let config = GrpcSourceConfig {
            ...
            descriptor_set: None,
            columns: Some(vec![
                ColumnConfig { name: "col1".to_string(), data_type: "UnknownType".to_string(), nullable: true },
            ]),
            tables: None,
        };
