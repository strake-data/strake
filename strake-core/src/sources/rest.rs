...
impl RestTableProvider {
    fn new(client: Arc<reqwest::Client>, config: RestSourceConfig, schema: arrow::datatypes::SchemaRef) -> Self {
...
impl RestExec {
    fn new(
        ...
        Self { client, config, schema: projected_schema, projection, limit, filters, cache }
    }
}
        ...
        let stream = futures::stream::try_unfold(
            ...
            move |(client, config, schema, next_url, current_offset, current_page, filters)| async move {
                 if let Some(mut url_str) = next_url {
                        ...
                        if let Some(pushdowns) = &config.pushdown {
                            if let Ok(mut url_obj) = url::Url::parse(&url_str) {
                                for filter in &filters {
                                    if let datafusion::logical_expr::Expr::BinaryExpr(datafusion::logical_expr::BinaryExpr { left, op, right }) = filter {
                                        if let (datafusion::logical_expr::Expr::Column(col), datafusion::logical_expr::Expr::Literal(val, _)) = (left.as_ref(), right.as_ref()) {
                                            let op_str = match op {
                     ...
                     if current_page >= 50 { // Safety break
                     ...
                     Ok(Some((batch, (client, config, schema, next_url_opt, new_offset, current_page + 1, filters))))
                 } else {
                     Ok(None)
                 }
            }
...
async fn infer_schema(client: &reqwest::Client, config: &RestSourceConfig) -> Result<arrow::datatypes::SchemaRef> {
    // 1. Fetch first page
    ...
    let resp = req.send().await.context("Failed to fetch API for schema inference")?;
...
async fn fetch_page(
    ...
    if let Some(PaginationConfig::Indices { param_offset, param_limit, limit: l, ..}) = &config.pagination {
        ...
        req = client.request(config.method.parse().unwrap_or(reqwest::Method::GET), url_obj.clone());
         for (k, v) in &config.headers {
            req = req.header(k, v);
        }
    ...
    if records.is_empty() {
         return Ok((arrow::record_batch::RecordBatch::new_empty(schema.clone()), None, new_offset));
    }
    if records.is_empty() {
         return Ok((arrow::record_batch::RecordBatch::new_empty(schema.clone()), None, new_offset));
    }
    tracing::debug!("RestExec: Sent request to {}, got {} records", url, records.len());
    ...
    let json_vals: Result<Vec<String>, _> = records.iter().map(|r| serde_json::to_string(r)).collect();
    ...
    let batch = arrow::compute::concat_batches(schema, &batches).context("Concat batches failed")?;
...
mod tests {
    ...
    async fn test_infer_schema() {
        ...
        let schema = infer_schema(&client, &config).await.expect("Infer schema failed");
    ...
    async fn test_fetch_page_pagination_header() {
        ...
        let config = RestSourceConfig {
            ...
            headers: HashMap::new(),
            auth: None,
            pagination: Some(PaginationConfig::Header { header_name: "Link".to_string() }),
            encoding: None,
            tables: None,
            pushdown: None,
        };
    ...
    async fn test_fetch_page_pagination_body() {
        ...
        let config = RestSourceConfig {
            ...
            headers: HashMap::new(),
            auth: None,
            pagination: Some(PaginationConfig::BodyUrl { path: "next".to_string() }),
            encoding: None,
            tables: None,
            pushdown: None,
        };
    ...
    async fn test_pushdown_filter() {
        ...
        let stream = exec.execute(0, Arc::new(datafusion::execution::TaskContext::default()))
