use strake_core::{config::*, federation::FederationEngine, auth::AuthenticatedUser};
...
async fn test_cache_speeds_up_repeated_query() -> anyhow::Result<()> {
    ...
    assert!(warnings1.contains(&"x-strake-cache: miss".to_string()), 
        "Expected cache miss, got warnings: {:?}", warnings1);
    ...
    let parquet_files: Vec<_> = cache_files.iter()
        .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("parquet"))
        .collect();
    assert_eq!(parquet_files.len(), 1, "Expected 1 cache file to be created");
    ...
    assert!(warnings2.contains(&"x-strake-cache: hit".to_string()),
        "Expected cache hit, got warnings: {:?}", warnings2);
...
async fn test_cache_isolation_by_user() -> anyhow::Result<()> {
    ...
    let cache_files: Vec<_> = std::fs::read_dir(cache_dir.path())?
        ...
        .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("parquet"))
        .collect();
    assert_eq!(cache_files.len(), 2, "Expected 2 cache files for different users");
...
async fn test_cache_disabled() -> anyhow::Result<()> {
    ...
    let cache_files: Vec<_> = std::fs::read_dir(cache_dir.path())?
        ...
        .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("parquet"))
        .collect();
    assert_eq!(cache_files.len(), 0, "Expected no cache files when cache is disabled");
...
async fn test_cache_large_dataset() -> anyhow::Result<()> {
    ...
    println!("   Memory size: {:.2} MB", total_bytes as f64 / 1024.0 / 1024.0);
    ...
    println!("   Cache file size: {:.2} MB", cache_file_size as f64 / 1024.0 / 1024.0);
    ...
    assert_eq!(batches[0].num_columns(), batches2[0].num_columns(), 
    ...
    println!("Cache file: {:.2} MB ({:.2}x compression)", 
        cache_file_size as f64 / 1024.0 / 1024.0, compression_ratio);
...
impl SourceProvider for MockProvider {
    fn type_name(&self) -> &'static str { "mock" }
...
async fn test_per_datasource_cache_config() -> anyhow::Result<()> {
    ...
    assert!(warnings1_hit.contains(&"x-strake-cache: hit".to_string()), 
        "Expected hit for enabled source");

    // 2. Query Uncached Source
    let sql_uncached = "SELECT * FROM uncached_source";
    let (_, _, warnings2) = engine.execute_query(sql_uncached, Some(user.clone())).await?;
    assert!(!warnings2.contains(&"x-strake-cache: miss".to_string()), 
