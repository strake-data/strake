use anyhow::Result;
use strake_core::config::{Config, SourceConfig, ResourceConfig};
...
async fn test_sqlite_joins() -> Result<()> {
    ...
    // Create users and orders in SQLite
    {
        ...
        conn.execute("INSERT INTO orders (user_id, amount) VALUES (?, ?)", rusqlite::params![1, 100.0])?;
        conn.execute("INSERT INTO orders (user_id, amount) VALUES (?, ?)", rusqlite::params![1, 50.0])?;
        conn.execute("INSERT INTO orders (user_id, amount) VALUES (?, ?)", rusqlite::params![2, 200.0])?;
    }
...
async fn test_sqlite_cross_db_federation() -> Result<()> {
    ...
    // DB 1: users
    {
        ...
        conn.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)", [])?;
        conn.execute("INSERT INTO users (id, name) VALUES (?, ?)", rusqlite::params![1, "Alice"])?;
    }

    // DB 2: user_details
    {
        let conn = rusqlite::Connection::open(&db2_path)?;
        conn.execute("CREATE TABLE user_details (user_id INTEGER PRIMARY KEY, age INTEGER)", [])?;
        conn.execute("INSERT INTO user_details (user_id, age) VALUES (?, ?)", rusqlite::params![1, 30])?;
    }
