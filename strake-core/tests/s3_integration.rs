...
async fn test_s3_source() {
    ...
    options.insert(
        "endpoint".to_string(),
        "http://denodocommunity-lab-environment-minio:9000".to_string(),
    );
    options.insert(
        "access_key_id".to_string(),
        "RFdpqH7thMJBuSAKkbS4".to_string(),
    );
    options.insert(
        "secret_access_key".to_string(),
        "S7GsoirzQLWackI1mwLVP0p2GSjnGGCFJ2ZfJzdD".to_string(),
    );
    ...
    provider
        ...
        .await
        .expect("Failed to register S3 source");

    let df = ctx
        .sql("SELECT * FROM strake.public.s3_test order by id")
        .await
