...
RUN ldd /build/target/x86_64-unknown-linux-musl/release/strake-cli || true
RUN ldd /build/target/x86_64-unknown-linux-musl/release/strake-server || true
RUN ldd /build/target/x86_64-unknown-linux-musl/release/strake-enterprise || true

# Strip binaries to reduce size
RUN strip /build/target/x86_64-unknown-linux-musl/release/strake-cli
RUN strip /build/target/x86_64-unknown-linux-musl/release/strake-server
RUN strip /build/target/x86_64-unknown-linux-musl/release/strake-enterprise

# ===== Runtime Stage =====
FROM scratch

# Copy binaries from builder
COPY --from=builder /build/target/x86_64-unknown-linux-musl/release/strake-cli /bin/strake-cli
COPY --from=builder /build/target/x86_64-unknown-linux-musl/release/strake-server /bin/strake-server
COPY --from=builder /build/target/x86_64-unknown-linux-musl/release/strake-enterprise /bin/strake-enterprise
